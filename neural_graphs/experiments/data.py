...
class PairBatch(NamedTuple):
    ...
    batch2: Batch
    def _assert_same_len(self):
        self.batch1._assert_same_len()
        self.batch2._assert_same_len()

    def as_dict(self):
        return {'batch1': self.batch1._asdict(),
                 'batch2': self.batch2._asdict()}

    def to(self, device):
        """move batch to device"""
        return self.__class__(
            batch1=self.batch1.to(device),
            batch2=self.batch2.to(device),
        )

    def __len__(self):
        return len(self.batch1.weights[0])

    def stack(self):
        weights = tuple([torch.cat((w1, w2), dim=0) for w1,w2 in zip(self.batch1.weights,
                                                                 self.batch2.weights)])
        biases = tuple([torch.cat((b1, b2), dim=0) for b1,b2 in zip(self.batch1.biases,
                                                                 self.batch2.biases)])
        labels = torch.cat((self.batch1.label, self.batch2.label), dim=0)
        return Batch(weights=weights, biases=biases, label=labels)
...
class INRDatasetSSL(INRDataset):
    def __init__(
        self,
        dataset_dir,
        splits_path,
        ...
        super().__init__(dataset_dir, splits_path, split=split,
                         normalize=normalize, permutation=permutation,
    ...
    def __getitem__(self, item):
        ...
        if len(self.transforms) > 1:
            weights1, biases1 = self.transforms[0](weights, biases)
            weights2, biases2 = self.transforms[1](weights, biases)
        else:
            weights1, biases1 = weights, biases
            weights2, biases2 = weights, biases
        # Add feature dim
        weights1 = tuple([w.unsqueeze(-1) for w in weights1])
        biases1 = tuple([b.unsqueeze(-1) for b in biases1])
        ...
        if self.permutation:
            weights1, biases1 = self._permute(weights1, biases1)
            weights2, biases2 = self._permute(weights2, biases2)

        return PairBatch(Batch(weights=weights1, biases=biases1, label=label),
                Batch(weights=weights2, biases=biases2, label=label))
...
def build_dataset(dataset_class, dataset_dir, splits_path, statistics_path):
    transforms1 = Compose([RandomTranslation(min_scale=0.1, max_scale=0.4),
                           RandomRotate(min_deg=10, max_deg=90),
                           RandomNoise(noise_min=1e-2, noise_max=1e-1),
                           Dropout(p=0.01),
                           RandomScale(min_scale=0.1, max_scale=0.4)])
    transforms2 = Compose([RandomTranslation(min_scale=0.1, max_scale=0.4),
                           RandomRotate(min_deg=10, max_deg=90),
                           RandomNoise(noise_min=1e-2, noise_max=1e-1),
                           Dropout(p=0.01),
                           RandomScale(min_scale=0.1, max_scale=0.4)])
