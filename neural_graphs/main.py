...
from experiments.utils import count_parameters, set_logger, set_seed, plot_fit, process_results_multi_gpu
...
ssl_dir = "checkpoints/siamse/exp0"
img_shape = (28, 28)
batch_size = 128
exp_num = 2
num_epochs = 200
...
if __name__ == "__main__":
    ...
    jobid = int(os.environ.get("SLURM_JOBID", "1"))
    ...
    state_dict = torch.load(f'{ssl_dir}/checkpoint.ckpt', map_location=torch.device('cpu'))
    new_state_dict = OrderedDict()
    for key, value in state_dict.items():
        while key.startswith('module.'):
            key = key[7:]
        if key.strip('module.').startswith("backbone."):
